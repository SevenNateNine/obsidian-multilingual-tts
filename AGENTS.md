...
`src/core/settings/migrations.ts` migrates the persisted settings schema, not a database. It runs on every load.
...
- To promise a caller that an array has a first element, type it `[T, ...T[]]`. See `AUDIO_FORMATS` in `src/adapters/azure/formats.ts` and `NonEmpty` in `src/core/audio/concat.ts`.

`exactOptionalPropertyTypes` makes `field?: T` reject an explicit `undefined`. When a field is cleared by assignment, declare it `field?: T | undefined`. See `VoiceProfile` in `src/core/settings/types.ts`.
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------- |
| File and folder layout | `src/` splits by dependency direction, not by feature. `core/` is policy: `settings/` schema and migration, `tts/` provider contracts, `audio/` playback and chunking, `text/` preparation and language detection, `usecases/` the read and save decisions. `adapters/` is detail, one folder per outside thing: `azure/`, `obsidian/`, `system/`. `ui/` is the Obsidian screens. Entry point is `src/main.ts`.    | `src/main.ts:1`                    |
| Naming                 | PascalCase file name when the file exports one class (`AudioPlayer.ts`, `ConvertModal.ts`). camelCase file name for a module of functions (`chunker.ts`, `paths.ts`). Types PascalCase, constants SCREAMING_SNAKE. Exception: `main.ts` is camelCase and exports a class, because Obsidian requires that entry point name. `errors.ts` exports a class and three functions, so it is a module and stays camelCase. | `src/core/audio/AudioPlayer.ts`    |
| Error handling         | Throw `TtsError` with a `TtsErrorKind`. Convert to text with `userMessage()` at the UI edge only. Swallow aborts with `isCancellation()`. Do not throw a bare `Error` from a speech path.                                                                                                                                                                                                                          | `src/core/errors.ts:19`            |
| Logging                | No logger and no log levels. A failure the user must see becomes `new Notice(userMessage(err))`. A failure a developer must debug becomes one `console.error` prefixed `[multilingual-tts]`, at the top-level catch only.                                                                                                                                                                                          | `src/main.ts:521`                  |
| Async model            | `async`/`await`. One `AbortController` per operation, its `AbortSignal` passed down to each provider call. Cancellation resolves, it does not reject. No timer used as control flow.                                                                                                                                                                                                                               | `src/core/audio/AudioPlayer.ts:67` |
| Test layout            | `*.test.ts` beside the file it tests. Vitest. `vitest.config.ts` aliases `obsidian` to `test/obsidian-stub.ts`, so only Obsidian-free logic is testable. Test the logic that fails quietly: text stripping, XML escaping, chunking, concatenation, path resolution, settings migration.                                                                                                                            | `src/core/paths.test.ts`           |
| Commit format          | Conventional Commits: `type(scope): subject`, for example `feat(azure): add style degree`. One commit exists so far, so almost no history confirms this.                                                                                                                                                                                                                                                           | none yet                           |
...
- Policy is all of `src/core/`. None of it imports `obsidian`, the speech SDK, or the language model, and `CORE_IMPORT_BOUNDARY` in `eslint.config.js` fails the build if it ever does.
- Detail is the Obsidian API, the Azure Speech SDK, the Web Speech API, and the vault file system.
- `RenderingProvider` and `SpeakingProvider` are declared in `src/core/tts/types.ts`, and implemented in `src/adapters/azure/` and `src/adapters/system/`.
- A provider type declares its own credential fields in `src/core/tts/providerTypes.ts`. Adding an engine is one entry there, one adapter class, and one branch in the factory in `src/main.ts`. Nothing in `src/ui/` may name an engine.
- Injected dependencies: `Fetcher` and `CatalogStore` in `src/adapters/azure/voiceCatalog.ts`, `Detector` in `src/core/text/detectLanguage.ts`, and the clock as a default parameter in `src/core/time.ts` and `src/core/paths.ts`.
...
| 2026-08-18 | Linter size limits      | `eslint.config.js`, `max-lines-per-function` off for `ProfileEditorModal.ts` and `*.test.ts` | The profile editor is one declarative builder chain. A test function is a list of assertions. Splitting either to meet a line budget hurts reading.                             | `SettingsTab.ts` no longer needs this: the General and Providers split broke it into sections that fit. Do the same to the profile editor if it grows a second section. |
